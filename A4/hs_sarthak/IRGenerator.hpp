    ...
    **/
    ...
    if(name == "local_decl") {
        ...
        if(root->children.size() == 3) {    // non-array declaration
            for(auto& child : root->children) {
                if(times[child->lexValue] > 1)
                    traverseAST(child);
            }
            if(times[root->children[1]->lexValue] > 1) {
        ...
        if(op_child->children.size() == 2) {
            string code;
            if(op_child->children[1]->isint)
                code = env.get(op_child->children[0]->lexValue) + " = " + to_string(op_child->children[1]->valuenow);
            else
    ...
    //    cout << "I am at " << name << endl;
        if(root->children.size() == 5) {     // if but no else
            if(root->children[2]->isint){
                if(root->children[2]->lexValue != "0")
     ...
     //    cout << "I am at " << name << endl;
        ...
        if(root->children.size() == 1) {   // binary op or Pexpr
            ...
            if(op_child->children.size() == 2) {     // binary op
                if(op == "AND") {
                    ...
                    if(!(op_child->children[0]->isint && op_child->children[1]->isint))
                        IR << code << endl;
                }
            }
            else {          // Pexpr
                ...
                root->isint = root->children[0]->isint;
                //cout << endl << endl << root->children[0]->addr << endl << root->children[0]->valuenow << endl << endl;
            }
        }
            ...
            if(op != "MINUS" || !root->children[1]->isint)
                IR << code << endl;
        }
        else {
       ...
       if(root->children.size() == 1) {
        ...
        if(root->children.size() == 3) {   // (expr)
            ...
            root->isint = root->children[1]->isint;
        }
        else {
