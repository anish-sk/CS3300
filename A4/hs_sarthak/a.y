...
%token NEWOP NEQ BITAND BITOR BITNOT BITXOR AND OR NOT MOD EXTERN LONG SHORT DOUBLE PRINTF PERCENTD SCANF
...
expr: Pexpr LT Pexpr    {
                            ...
                            vector<treeNode*> v = {$2};
                            $$ = new treeNode("expr", v);
                            $$->isint = $1->isint && $3->isint;
                            bool temp = ($1->valuenow < $3->valuenow);
                            if(temp) $$->valuenow = 1;
                            else $$->valuenow = 0;
                            $$->lexValue = to_string($$->valuenow);
                        }
    | Pexpr GT Pexpr    {   
                            vector<treeNode*> u = {$1, $3};
                            $2 = new treeNode("GT", u);
    ...
    | Pexpr OR Pexpr    {
                            ...
                            $$ = new treeNode("expr", v);
                            $$->isint = $1->isint && $3->isint;
                            bool temp = ($1->valuenow != 0 || $3->valuenow != 0);
                            if(temp) $$->valuenow = 1;
                            else $$->valuenow = 0;
                            $$->lexValue = to_string($$->valuenow);
                        }
    ...
    | Pexpr AND Pexpr   {
                            ...
                            $$ = new treeNode("expr", v);
                            $$->isint = $1->isint && $3->isint;
                            bool temp = ($1->valuenow * $3->valuenow != 0);
                            if(temp) $$->valuenow = 1;
                            else $$->valuenow = 0;
                            $$->lexValue = to_string($$->valuenow);
                        }
    ...
    | Pexpr PLUS Pexpr  {
                            ...
                            $$ = new treeNode("expr", v);
                            $$->isint = $1->isint && $3->isint;
                            $$->valuenow = $1->valuenow + $3->valuenow;
                            $$->lexValue = to_string($$->valuenow);
                        }
    | Pexpr MINUS Pexpr {
                            vector<treeNode*> u = {$1, $3};
...
integerLit: INTEGER_NUMBER  {
                                ...
                                $$->lexValue = mytext;
                                $$->valuenow = atoi(mytext);
                                $$->isint = true;
                                $$->ispow2 = (ceil(log2($$->valuenow)) == floor(log2($$->valuenow)));
                                $$->width = 4;
                            };
