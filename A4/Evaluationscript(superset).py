...
match_lines = ["unused-vars","if-simpl","strength-reduction","constant-folding","constant-propagation","cse"]

def parse_summary(lines):
    '''
        Finds the line nos to break the summary file
    '''
    idxs = []
    j = 0
    ...
    # print(idxs)
    if(len(idxs) != len(match_lines)):
        return -1

    return idxs

def check_ordered_subset(lines1,lines2):
    '''
        Checks if lines2 forms the subset of lines1
        if true it will return no. of extra lines
        else returns -1
    '''
    if(len(lines2) == 0):
        return len(lines1)
    ...
    i,j = 0,0
    while(i < len(lines2)):
        while((j<len(lines1)) and (lines2[i] != lines1[j])):
            j += 1
        if(j == len(lines1)):
            break
    ...
    # print(lines1,lines2)
    if((j==len(lines1)) and (i!=len(lines2))):
        return -1
    else:
        return len(lines1) - len(lines2)


def check_summary(file_path1,file_path2):
    # print(file_path1,file_path2)
    file1 = open(file_path1,"r")
    file2 = open(file_path2,"r")
    
    # Filter out non empty lines
    file1_lines = list((line.strip() for line in file1.readlines() if line.strip()))
    file2_lines = list((line.strip() for line in file2.readlines() if line.strip()))

    # print(file1_lines)
    # print(file2_lines)

    # Gets indexes of lines to break
    idxs1 = parse_summary(file1_lines)
    idxs2 = parse_summary(file2_lines)

    if((idxs1 == -1) or (idxs2 == -1)):
        # print(idxs1,idxs2)
        return -1,None

    # Just to get ranges directly from indexes
    idxs1.append(len(file1_lines))
    idxs2.append(len(file2_lines))
    
    passed = True

    extra_lines = 0
    for i in range(len(match_lines)):
        temp = check_ordered_subset(file1_lines[idxs1[i]+1:idxs1[i+1]],file2_lines[idxs2[i]+1:idxs2[i+1]])
        if(temp == -1):
            passed = False
            break
        extra_lines += temp
...
class TestRunner:
    ...
    def run(self, bin_path, subm_path):
        ...
        exefile_path = os.path.join(subm_path, 'hope.out')
        os.chdir(subm_path)
        ...
        for test in self.testcases:
            ...
            ret1, out1 = check_summary(smfile_path,test["summary"])
            # print(ret,ret1,out,out1)
            if ret != 0 or ret1 != 0 or out is None or out1 is None or len(out) > 0 or len(out1) > 0:
                if(ret != 0):
                    print_info(err)
                success = False
            else:
